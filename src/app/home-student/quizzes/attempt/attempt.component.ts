...
export class AttemptComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userService.getStudent(this.appService.getToken().id).subscribe(
      (res:any)=>{
  ...
  save(correctAnswers:number,answers:String[]){
    ...
    let status="";
    if(correctAnswers>=(this.questions.length/2))
      status="Pass";
    else
      status="Fail";
    ...
    let attempt:any={
      ...
      answers:answers,      
      taken_on: date.getFullYear()+"-"+date.getMonth()+"-"+date.getDate()
    }
  ...
  validate(){
    ...
    this.questions.forEach((question,i)=>{
      let userAnswer = (answerContainers[i].querySelector('input[name=question'+(i+1)+']:checked')||{}).value;
      if(userAnswer!=null)
    ...
    this.questions.forEach((question,j)=>{
      ...
      for(let i=1;i<=4;i++){
        ...
        if(i==1){
          answers.push(
            ...
            '<label style="cursor:pointer;" >'
              + '<input  type="radio" name="question'+(j+1)+'" value="'+val+'"/>'
              + "<p  class='d-inline text-white ml-2'>" +val+" : "+question.optionA+"</p>"
            + '</label>'
            +"</div>"
          )
        }
        ...
        answers.push(
          ...
          '<label style="cursor:pointer;">'
            + '<input type="radio" name="question'+(j+1)+'" value="'+val+'"/>'
            + "<p class='d-inline text-white ml-2'>" +val+" : "+question.optionB+"</p>"
          + '</label>'
          +"</div>"
          +"</div>"
          ...
          answers.push(
            ...
            '<label style="cursor:pointer;">'
              + '<input type="radio" name="question'+(j+1)+'" value="'+val+'">'
              + "<p class=' d-inline text-white ml-2'>" +val+" : "+question.optionC+"</p>"
            + '</label>'
            +"</div>"
        ...
        answers.push(
          ...
          '<label style="cursor:pointer;">'
            + '<input type="radio" name="question'+(j+1)+'" value="'+val+'">'
             + "<p class=' d-inline text-white ml-2'>" +val+" : "+question.optionD+"</p>"
          + '</label>'
          +"</div>"
          +"</div>"
      ...
      output.push(
        ...
        +"</label>"
        +"</div>"
        +" <label class='h5 text-white mt-2'  for='options'> Statement: "+questionStatement+" </label>"
        +"  <div class='row mt-4'>"
        +"<div class='col-md-12'>"
