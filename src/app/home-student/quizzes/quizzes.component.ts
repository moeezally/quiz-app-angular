...
export class QuizzesComponent implements OnInit {
  ...
  findAttempts(Id:any){
    this.attemptService.getStudentAttempts(Id).subscribe(
      (res:any)=>{
        console.log(res);          
  ...
  setModalData(id:any){
    ...
    if(name!=null)
    name.innerHTML=attempt.quiz_id.name;
    let status=document.getElementById("attempt_status");
    if(status!=null)
    status.innerHTML=attempt.status;
    ...
    if(taken_on!=null)
    taken_on.innerHTML=attempt.taken_on;
