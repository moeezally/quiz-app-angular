...
export class TeacherQuizzesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (token != null) {
      ...
      this.quizService
        .getTeacherQuizzes(parsedToken.id)
        .subscribe((response: any) => {
          this.quizzes = response.data.quizzes;
          this.isLoading = false;
        });
      this.attemptService.getAll().subscribe((res: any) => {
  ...
  setModalData(id: any) {
    ...
    let quizAttempts = this.attempts.filter((attempt) => {
      if (attempt.quiz_id.id == quiz.id) return attempt;
    });
    ...
    if (quizAttempts.length == 0) outputHTML.push('0 Attempts');

    console.log(outputHTML.join(''));

    let tbody = document.getElementById('attempt_table_body');
    if (tbody != null) tbody.innerHTML = outputHTML.join('');
  }
}
